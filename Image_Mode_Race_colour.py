...
def get_gender_count(image, face_net, gender_net, confidence_threshold=0.5):
    ...
    faces, _ = detect_and_crop_faces(image, face_net, confidence_threshold)
...
def get_image_detail(image):
    ...
    result = DeepFace.analyze(img_path=image, actions=["gender", "race"], enforce_detection=False)
    races = []
    for entry in result:
        dominant_race = entry['dominant_race']
        races.append(dominant_race)
    return races
...
# Detect faces and crop them
def detect_and_crop_faces(image, confidence_threshold=0.5):
    detector = MTCNN()
    results = detector.detect_faces(image)
...
def get_gender_count(image,gender_net, confidence_threshold=0.5):
    ...
    for face_image in faces:
        ...
        predictions = gender_net.forward()
        Skin.append(extract_skin_regions(face_image))
        Races.appens(get_image_detail(face_image))
